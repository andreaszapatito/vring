...
!    call commq(var,var%su%qrhs,2)
!    call commq(var,var%su%qrhs,3)
...
!        if(var%com%ip_a(2).eq.var%com%np_a(2)-1) var%su%dq3(ic,var%msh%nrc,kc)=var%su%dq3(ic,var%msh%nr,kc)
      enddo
    enddo
...
!         RHS of First pass of the factorisation AF1 for q2
    ...
    do ic=1,var%msh%ntheta
      do kc=1,var%msh%nzc
        if(var%com%ip_a(2).eq.var%com%np_a(2)-1) var%su%dq2(ic,var%msh%nrc,kc)=0.0*var%par%slip*var%su%dq2(ic,var%msh%nr,kc)
      enddo
    enddo
...
!      if (ieq.eq.3) then
!      if(var%msh%rc(jc).le.0.1999.and.var%com%ip_a(3).eq.0) var%su%rtri(1)=uin                          ! quand Vtheta_sud impose (voir coesys.f)
